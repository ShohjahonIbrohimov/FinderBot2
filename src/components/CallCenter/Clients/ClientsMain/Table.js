...
export default function FinanceTable({ finance_data, handleDelete }) {
    ...
    const columns = React.useMemo(
        () => [
            ...
            {
                Header: <><span>Имя/компания</span><img className="long_arrow_up" src={call_center_icons.long_arrow_up} alt="" /></>,
                accessor: "name",
            },
            {
                Header: <><span>Электронная почта</span><img className="table_arrow_right" src={call_center_icons.up_down_arrow_icon} alt="" /></>,
                accessor: "login",
            },
            {
                Header: <><span>Телефон</span><img className="table_arrow_right" src={call_center_icons.up_down_arrow_icon} alt="" /></>,
                accessor: "phone"
            },
            {
                Header: <><span>Адрес</span><img className="table_arrow_right" src={call_center_icons.up_down_arrow_icon} alt="" /></>,
                accessor: "adress",
            },
            {
                Header: <><span>Рекламная компания</span><img className="table_arrow_right" src={call_center_icons.up_down_arrow_icon} alt="" /></>,
                accessor: "reklama",
            },
            {
                Header: <><span>Примичание</span><img className="table_arrow_right" src={call_center_icons.up_down_arrow_icon} alt="" /></>,
                accessor: "note",
            },
            {
                Header: <><span>Баланс, руб</span><img className="table_arrow_right" src={call_center_icons.up_down_arrow_icon} alt="" /></>,
                accessor: "price",
            },
        ],
        []
        ...
        <div className="webmaster_table my_client_table">
                                    ...
                                    {row.cells.map((cell) => {
                                        return (
                                            <td {...cell.getCellProps()} className="td">
                                                <span className="span-td">{getCustomContent(cell)}</span>
                                            </td>
                                        );
                                    })}
                                </tr>
            ...
            <div className="actions_row">
                <div className="sub_actions_row1">
                    ...
                    <span style={{ marginLeft: "2rem", fontWeight: "500", display: "flex", alignContent: "center", flexWrap: "wrap" }}>Выбрано — 1</span>
                </div>
            ...
            <div className="webmaster_pagination">
                <button
                    className={!canPreviousPage ? "mr-16 pagination_btn1_disabled" : "mr-16 pagination_btn1"}
                    onClick={() => previousPage()}
                    disabled={!canPreviousPage}
                >
                    <img className="webm_arrow_left" src={webmaster_icons.pagination_arrow_left} alt="" />
                </button>
                {
                    pageOptions.map((item) => {
                        return <button
                            ...
                            onClick={e => {
                                const page = e.target.innerText ? Number(e.target.innerText) - 1 : 0
                                gotoPage(page)
                ...
                <i className="dot3">&nbsp; . . . &nbsp;</i>
                <button
                    className={pageOptions.length == pageIndex ? "btn_disabled" : "btn_page end"}
                ...
                <button
                    className={!canNextPage ? "mr-16 pagination_btn1_disabled" : "mr-16 pagination_btn2"}
                    onClick={() => nextPage()}
                    disabled={!canNextPage}
                >
                    <img className="webm_arrow_right" src={webmaster_icons.pagination_arrow_right} alt="" />
                </button>
            </div>
        </div>
    );
